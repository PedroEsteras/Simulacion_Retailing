...
from src.models.transactions.base import Transaction
from src.models.utils import safe_log
import json
...
class Model(object):
    ...
    def log_likelihood_for(self, transactions):
        ...
        return result / len(transactions) - self.regularization_penalty()

    def regularization_penalty(self):
        # does not implement any regularization; subclasses should do it
        return 0
    ...
    def chi_squared_score_for(self, transactions):
        """
        Calculates the approximate chi squared metric as defined in Jagabathula, S., & Vulcano, G. (2018). A partial-order-based model to estimate individual preferences using panel data. Management Science, 64(4), 1609–1628.
        :param transactions: the transactions over which to calculate the approximate chi squared prediction score
        :return: the approximate chi squared score
        """
        expected_purchases = [0.0 for _ in self.products]
        observed_purchases = [0.0 for _ in self.products]
        ...
        for transaction in transactions:
            ...
            for product in transaction.offered_products:
                probabilities.append(
                    (product, self.probability_of(Transaction(product, transaction.offered_products))))
            most_probable = max(probabilities, key=lambda p: p[1])[0]
            expected_purchases[most_probable] += 1.0

        score = 0.0
        for p in self.products:
            score += (((expected_purchases[p] - observed_purchases[p]) ** 2) / (expected_purchases[p] + 0.5))
        score = score / float(len(self.products))
...
from src.models.models.exponomial import ExponomialModel
from src.models.models.latent_class import LatentClassModel
from src.models.models.markov_chain import MarkovChainModel
from src.models.models.markov_chain_rank_2 import MarkovChainRank2Model
from src.models.models.multinomial_logit import MultinomialLogitModel
from src.models.models.nested_logit import NestedLogitModel
from src.models.models.ranked_list import RankedListModel
from src.models.models.random_choice import RandomChoiceModel
from src.models.models.mixed_logit import MixedLogitModel
from src.models.models.generalized_stochastic_preference import GeneralizedStochasticPreferenceModel
from src.models.models.multinomial_logit_reg import MultinomialLogitRegModel
