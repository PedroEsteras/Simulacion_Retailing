...
from src.models.optimization.non_linear import NonLinearProblem, NonLinearSolver, Constraints
from src.models.profiler import Profiler
from src.models.settings import Settings
from src.models.utils import safe_log, ZERO_LOWER_BOUND, ONE_UPPER_BOUND, time_for_optimization
...
class LatentClassFrankWolfeEstimator(Estimator):
    ...
    def look_for_new_mnl_model(self, model, likelihood_loss_function_coefficients):
        ...
        mnl_sb_profiler = Profiler(verbose=False)
        # solution = NonLinearSolver.default().solve(problem, self.profiler())  ## instead of reusing the profiler...
        solution = NonLinearSolver.default().solve(problem, mnl_sb_profiler)  ## ... start a new one to avoid timeouts with very few iterations
    ...
    def update_weights_for(self, model, likelihood_loss_function_coefficients):
        ...
        weights_sb_profiler = Profiler(verbose=False)
        # solution = NonLinearSolver.default().solve(problem, self.profiler()) ## instead of reusing the profiler...
        solution = NonLinearSolver.default().solve(problem, weights_sb_profiler)  ## ... start a new one to avoid timeouts with very few iterations
    ...
    def estimate(self, model, transactions):
        ...
        for _ in range(max_iterations):
            ...
            self.profiler().stop_iteration(new_likelihood)
