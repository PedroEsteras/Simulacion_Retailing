...
from src.models.utils import finite_difference, time_for_optimization
import time
...
class ScipySolver(NonLinearSolver):
    ...
    def solve(self, non_linear_problem, profiler):
        ...
        def iteration_callback(x, state=None):
        ...
        if not success: # try different optimization method

            profiler.start_iteration()
            try:
                print('>>>>>>>>>>>>>>>>>>>>>Failed to optimize with SLSQP. Attempting with trust-constr.<<<<<<<<<<<<<<<<<<<<')
                MAX_ITER = 100000/100
                r = minimize(fun=non_linear_problem.objective_function, x0=array(non_linear_problem.initial_solution()),
                             jac=False, bounds=bounds, constraints=constraints, callback=iteration_callback,
                             method='trust-constr', tol=1e-8, options={'maxiter': MAX_ITER})
...
class NonLinearProblem(object):
    ...
    def jacobian(self, vector):
        # inherited TO DO from original code: Is it bad to define this 'finite difference' function each time jacobian is called?
